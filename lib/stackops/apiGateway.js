...
const stageMethodConfigMappings = {
	...
	dataTraceEnabled: {
		prop: "DataTraceEnabled",
		validate: _.isBoolean,
		default: false,
	},
	loggingLevel: {
		prop: "LoggingLevel",
		validate: (value) => _.includes(["OFF", "INFO", "ERROR"], value),
		default: "OFF",
	},
	metricsEnabled: {
		prop: "MetricsEnabled",
...
const internal = {
	...
	createStageResource(restApiRef, deploymentName) {
		...
		const serviceName = _.get(
			this.serverless.service.getServiceObject() || {},
			"name"
		);
		stageResource.Properties.Description = `Alias stage '${this._alias}' for ${serviceName}`;

		// Configure stage (service level)
		const serviceLevelConfig = _.cloneDeep(
			_.get(
				this.serverless.service,
				"custom.aliasStage",
				_.get(this.serverless.service, "provider.aliasStage", {})
			)
		);
		if (serviceLevelConfig.cacheClusterEnabled === true) {
			...
			if (_.has(serviceLevelConfig, "cacheClusterSize")) {
				stageResource.Properties.CacheClusterSize =
					serviceLevelConfig.cacheClusterSize;
			}
		}
		...
		_.forEach(functions, (funcName) => {
			...
			const funcStageConfig = _.defaults(
				{},
				func.aliasStage,
				serviceLevelConfig
			);
			const funcHttpEvents = _.compact(
				_.map(
					this.serverless.service.getAllEventsInFunction(funcName),
					(event) => event.http
				)
			);

			_.forEach(funcHttpEvents, (httpEvent) => {
				const eventStageConfig = _.defaults(
					{},
					httpEvent.aliasStage,
					funcStageConfig
				);
				if (!_.isEmpty(eventStageConfig)) {
					...
					_.forOwn(eventStageConfig, (value, key) => {
						if (!_.has(stageMethodConfigMappings, key)) {
							throw new this.serverless.classes.Error(
								`Invalid stage config '${key}' at method '${methodType} /${httpEvent.path}'`
							);
						} else if (!stageMethodConfigMappings[key].validate(value)) {
							throw new this.serverless.classes.Error(
								`Invalid value for stage config '${key}: ${value}' at method '${methodType} /${httpEvent.path}'`
							);
						}
						if (
							!_.has(stageMethodConfigMappings[key], "default") ||
							stageMethodConfigMappings[key].default !== value
						) {
					...
					if (!_.isEmpty(methodSetting)) {
						methodSetting.ResourcePath =
							"/" +
							_.replace("/" + _.trimStart(httpEvent.path, "/"), /\//g, "~1");
...
module.exports = function (
	...
	const stageStack =
		this._serverless.service.provider.compiledCloudFormationTemplate;
	const aliasStack =
		this._serverless.service.provider.compiledCloudFormationAliasTemplate;
	const userResources = _.get(this._serverless.service, "resources", {
		Resources: {},
		Outputs: {},
	});
	...
	if (!exposeApi) {
		...
		if (
			_.some(aliasStackTemplates, (template) =>
				_.find(template.Resources, ["Type", "AWS::ApiGateway::Deployment"])
			)
		) {
			// Fetch the Api resource from the current stack
			stageStack.Resources.ApiGatewayRestApi =
				currentTemplate.Resources.ApiGatewayRestApi;
			exposeApi = true;
		}
	}
	...
	if (exposeApi) {
		...
		if (
			...
			_.find(currentAliasStackTemplate.Resources, [
				"Type",
				"AWS::ApiGateway::Method",
			])
		) {
			throw new this._serverless.classes.Error(
				"ALIAS PLUGIN ALPHA CHANGE: APIG deployment had to be changed. Please remove the alias stacks and the APIG stage for the alias in CF (AWS console) and redeploy. Sorry!"
			);
		}

		// Move the API deployment into the alias stack. The alias is the owner of the APIG stage.
		const deployment = _.assign(
			{},
			_.pickBy(stageStack.Resources, ["Type", "AWS::ApiGateway::Deployment"])
		);
		if (!_.isEmpty(deployment)) {
			...
			const stageResource = internal.createStageResource.call(
				this,
				`${stackName}-ApiGatewayRestApi`,
				deploymentName
			);
			aliasResources.push({ ApiGatewayStage: stageResource });

			const baseMapping = _.assign(
				{},
		...
		const apiLambdaPermissions = _.assign(
			{},
			_.pickBy(
				_.pickBy(stageStack.Resources, ["Type", "AWS::Lambda::Permission"]),
				(permission) => utils.hasPermissionPrincipal(permission, "apigateway")
			)
		);

		const apiMethods = _.assign(
			{},
			_.pickBy(stageStack.Resources, ["Type", "AWS::ApiGateway::Method"])
		);
		const authorizers = _.assign(
			{},
			_.pickBy(stageStack.Resources, ["Type", "AWS::ApiGateway::Authorizer"])
		);
		const aliases = _.assign(
			{},
			_.pickBy(aliasStack.Resources, ["Type", "AWS::Lambda::Alias"])
		);
		const versions = _.assign(
			{},
			_.pickBy(aliasStack.Resources, ["Type", "AWS::Lambda::Version"])
		);
		...
		_.forOwn(apiMethods, (method, name) => {
			...
			if (
				_.includes(
					["AWS", "AWS_PROXY"],
					_.get(method, "Properties.Integration.Type")
				)
			) {
			...
			// Remove unneeded dependency (added by serverless v3)
			_.remove(method.DependsOn, (dep) =>
				dep.endsWith("LambdaPermissionApiGateway")
			);
		...
		const isExternalRefAuthorizerPredicate = (part) =>
			_.startsWith(part, "arn:aws:lambda") ||
			(_.has(part, "Fn::Sub") &&
				_.startsWith(part["Fn::Sub"], "arn:aws:lambda"));
		_.forOwn(authorizers, (authorizer, name) => {
			...
			if (authorizerType === "TOKEN" || authorizerType === "REQUEST") {
				...
				const isExternalRefAuthorizer = _.some(
					uriParts,
					isExternalRefAuthorizerPredicate
				);
				if (!isExternalRefAuthorizer) {
					const funcIndex = _.findIndex(uriParts, (part) =>
						_.startsWith(part, "/invocations")
					);
			...
			_.forOwn(stageStack.Resources, (resource) => {
				if (_.isString(resource.DependsOn) && resource.DependsOn === name) {
					...
					_.isArray(resource.DependsOn) &&
					_.includes(resource.DependsOn, name)
				) {
		...
		_.forOwn(apiLambdaPermissions, (permission, name) => {
			...
			const isExternalRef = isExternalRefAuthorizerPredicate(
	...
	_.forEach(aliasResources, (resource) =>
		_.assign(aliasStack.Resources, resource)
	);
